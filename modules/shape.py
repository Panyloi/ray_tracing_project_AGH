...
class Shape:
    ...
    def updatePixelMap(
        self, image: Image, cords: Vec3 = Vec3(0, 0, 0), color: Vec3 = BLACK
    ):
...
class Sphere(Shape, Hittable):
    ...
    def get_sphere_color(self):
        r = self.sphere_color.x
        g = self.sphere_color.y
        b = self.sphere_color.z
...
class Cuboid(Shape):
    ...
    def contains(self, x, y):
        ...
        return (
            ...
            and x <= self.center.x + self.width / 2
            and y >= self.center.y - self.height / 2
            and y <= self.center.y + self.height / 2
            and self.center.z - self.depth / 2 <= 0
            and self.center.z + self.depth / 2 >= 0
        )
